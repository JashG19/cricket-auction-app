 ...
 */
export const exportToCSV = (teams, players, filename = 'auction_results.csv') => {
  try {
    const data = []
    ...
    data.push([])
    data.push(['Team Name', 'Owner', 'Total Budget', 'Spent', 'Remaining', 'Players Count'])
    ...
    data.push([])
    data.push(['Player Name', 'Age', 'Group', 'Base Price', 'Sold Price', 'Sold To'])
 ...
 */
export const exportToExcel = (teams, players, filename = 'auction_results.xlsx') => {
  try {
    ...
    const summaryData = [
      ...
      },
      {
        ...
        'Value': teams.reduce((sum, team) => sum + (team.budget_total - team.budget_remaining), 0),
      },
    ]
 ...
 */
export const exportTeamSquads = (teams, players, filename = 'team_squads.xlsx') => {
  try {
    const workbook = XLSX.utils.book_new()
    ...
    teams.forEach((team) => {
      ...
      XLSX.utils.book_append_sheet(workbook, sheet, team.team_name.slice(0, 31))
    })
...
export const generateAuctionSummary = (auctionData, teams, players) => {
  return {
    ...
    totalSpent: teams.reduce((sum, team) => sum + (team.budget_total - team.budget_remaining), 0),
    teamsData: teams.map((team) => ({
      ...
      squadPlayers: team.squad?.map((pid) => {
